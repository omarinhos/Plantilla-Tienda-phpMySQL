...
if (is_array($datos)){
    ...
    $sql = $con -> prepare("INSERT INTO venta (id_transaccion, fecha, status, email, id_cliente, total) 
    VALUES (?,?,?,?,?,?)");
    ...
    if ($id > 0) {
        ...
        if ($productos != null) {
            foreach ($productos as $clave => $cantidad) {
                ...
                $sql_insert = $con->prepare("INSERT INTO detalle_venta (id_venta, id_producto, nombre, precio, cantidad) 
                VALUES (?,?,?,?,?)");
